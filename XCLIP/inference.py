...
import pickle
from sklearn.metrics import accuracy_score, top_k_accuracy_score, precision_recall_fscore_support
...
def validate(val_loader, text_labels, model, config):
    ...
    #acc1_meter, acc5_meter, mAP_meter, AUC_meter, bAcc_meter, MPrecision_meter, MF1_meter = AverageMeter(), AverageMeter(),AverageMeter(), AverageMeter(),AverageMeter(), AverageMeter(), AverageMeter()
    ...
    with torch.no_grad():
        ...
        for idx, batch_data in enumerate(val_loader):
            ...
            acc1, acc5 = 0, 0
            for i in range(b):
                # logger.info(f'{label_id[i]}, {indices_1[i]}, {tot_similarity[i]}\t')
            ...
            y_scores.append(tot_similarity.cpu().numpy())
            y_true.append(label_id.cpu().numpy())
            y_pred.append(indices_1.cpu().numpy())
            #index.append(idx)
            ...
            # G-Means
            # g_means = g_mean(y_true, y_pred)

            #mAP_meter.update(mAP, b) 
            #AUC_meter.update(auc, b) 
            #bAcc_meter.update(balanced_acc, b)
            #MPrecision_meter.update(macro_precision, b)
            #MF1_meter.update(macro_f1, b)    
    ...
    #logger.info(f' * Acc@1 {acc1_meter.avg:.3f} Acc@5 {acc5_meter.avg:.3f} mAP {mAP_meter.avg:.3f} AUC {AUC_meter.avg:.3f} bAcc {bAcc_meter.avg:.3f} MPrecision {MPrecision_meter.avg:.3f} MF1 {MF1_meter.avg:.3f}')
